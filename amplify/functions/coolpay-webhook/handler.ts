...
export const handler = async (event: any) => {
  ...
  if (transaction_type === 'PAYOUT') {
      ...
      if (!payoutIntent) return { statusCode: 200, body: 'OK (intent retrait introuvable)' };
      ...
      if (payoutIntent.isPlatformWithdrawal) { // NOUVEAU : branche retrait S.Kalify
        if (transaction_status === 'FAILED' || transaction_status === 'CANCELED') {
          ...
          if (platformBalance) {
            await ddb.send(new UpdateCommand({
              ...
              ExpressionAttributeValues: { ':newAmount': (platformBalance.amount ?? 0) + payoutIntent.debitedAmount },
            }));
            ...
            await ddb.send(new PutCommand({
              ...
              Item: {
                ...
                reason: 'Remboursement retrait admin échoué (My-CoolPay)',
        ...
        // Cas SUCCESS : déjà débité et déjà tracé côté client, rien de plus à faire ici

      } else { // NOUVEAU : branche retrait utilisateur — logique existante, strictement inchangée
